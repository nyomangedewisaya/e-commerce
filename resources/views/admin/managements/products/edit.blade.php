    ...
    <div x-data="{ loaded: false }" x-init="setTimeout(() => loaded = true, 200)">
        <div x-show="loaded" x-cloak x-transition:enter="transition-all ease-out duration-1000"
            ...
            <div class="container mx-auto">
                ...
                <form action="{{ route('managements.products.update', $product->slug) }}" method="POST" enctype="multipart/form-data"
                    class="space-y-4">
                    @csrf
                    @method('PUT')
                    <div>
                    ...
                    <div>
                        ...
                        <textarea name="description" id="description" rows="3" placeholder="Masukan deskripsi untuk produk ini..."
                            class="bg-gray-100 w-full rounded-lg shadow-lg border border-gray-300 px-4 py-2 outline-none">{{ old('description', $product->description) }}</textarea>
                    </div>
                    @error('description')
                        <p class="text-xs text-red-700">{{ $message }}</p>
                    @enderror
